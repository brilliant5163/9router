...
function getSystemTailscaleCommand() {
  return IS_WINDOWS ? "where tailscale 2>nul" : "command -v tailscale 2>/dev/null";
}
...
function bgRefreshBin() {
  ...
  execAsync(getSystemTailscaleCommand(), { windowsHide: true, timeout: PROBE_TIMEOUT_MS })
    .then(({ stdout }) => {
